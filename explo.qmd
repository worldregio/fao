...
```{r}
code_items<-read.csv2("data/FBS_codes.csv") %>% select(1:8) %>% unique()
...
### Global Food Balance in 2022

We can firstly try to compute some global values, for example in 2022.

#### World level

```{r}
base <-readRDS("data/base_fao_Kcal_2010_2022.RDS")
x<-base %>% filter(t=="2022") %>%
             group_by(s) %>%
           ...
           mutate(K=KP/P)
kable(x, caption = "Food ration in Kcal by day at world level in 2022",col.names = c("Total Kcal","Total Population", "Kcal/capita")
      )
```

- **Comment** : If our results are correct, the average food ration at world level (weighted by population of countries) is equal to **2395 kcal/capita/day**. This value is based on 190 coutries that summarize a total of 7.915 billions of inhabitants. 
...
```{r}
base <-readRDS("data/base_fao_Kcal_2010_2022.RDS")
x<-base %>% filter(t=="2022", is.na(cont_s)==F) %>%
             group_by(s,cont_s) %>%
            summarise(K=sum(Kist),
                      P = max(Pst)) %>%
            mutate(KP=K*P) %>%
  ...
  arrange(Index)
kable(x, caption = "Food ration in Kcal by day at continental level in 2022",
      col.names = c("Continent","Kcal","Population","% Kcal","% Pop", "Kcal/capita", "index 100 = World"),
      digits=c(0,0,0,2,2,0)
      )
```

- **Comment** : At continental level, we add some columns in order to compare for each continent the share of population and the share of food consumption measured in Kcal. According to our results, the lowest level of consumption is observed in **Asia** which account for 59.1% of world population but only 51.9% of food consumption mesured in Kcal. The level of consumption of Asia is at index 88 i.e -12% lower than the world average. On the contrary, Europe is at index 140 which is +40% higher than the world average. 
...
kable(x, caption = "Food ration in Kcal by day at regional level in 2022",
      col.names = c("region","Kcal","Population","% Kcal","% Pop", "Kcal/capita", "index 100 = World"),
      digits=c(0,0,0,2,2,0)
      )
```

- **Comment** : We can replicate the same analysis at the level of the 22 regions defined by UN as subdivisions of continents. We can reveal more important inequalities with a lowest level of 79 (-21% of world average) in southern Asia and an highest level of 155 (+55% of world average) in Northern America. These results are consistent with the publication of the FAO and the litterature on the subject.
